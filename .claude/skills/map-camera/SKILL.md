...
and a table across the bottom is a spot that is half covered.
...
reading geometry off the **live** furniture each time. It has to be live: furniture can undock,
...
bottom padding). A static edge string goes stale the moment something undocks, docks again, or is
...
A band spanning nearly the full viewport width — this app's own table, inset 10px on left,
...
## An undocked piece of furniture counts for nothing

Furniture parked away from every edge — an undocked panel dragged to the middle of the map —
